...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (api === "hr_leave_my") {
      ...
      if(reqIds.length){ const { data: allSteps } = await sb.from("hr_leave_approval_steps").select("*").in("leave_request_id",reqIds).order("step_order"); await attachActorNames(allSteps||[], "decided_by", "decided_by_name"); (allSteps||[]).forEach((s:any)=>{ (stepsByReq[s.leave_request_id]=stepsByReq[s.leave_request_id]||[]).push(s); }); }
    ...
    if (api === "hr_leave_admin") {
      ...
      if(ids.length){ const { data: st } = await sb.from("hr_leave_approval_steps").select("*").in("leave_request_id",ids).order("step_order"); await attachActorNames(st||[], "decided_by", "decided_by_name"); (st||[]).forEach((s:any)=>{ (stepsByReq[s.leave_request_id]=stepsByReq[s.leave_request_id]||[]).push(s); }); }
    ...
    if (api === "hr_rc_get") {
      ...
      await attachActorNames(allSteps, "acted_by", "acted_by_name");   // who approved each step → timeline
