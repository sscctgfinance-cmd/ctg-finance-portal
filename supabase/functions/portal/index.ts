...
function myServiceMonths(emp:any,period:any){ if(!period||!period.year) return 12; const y=Number(period.year); let start=1,end=12;
...
function computePayrollMY(emp:any, cfg:any, adj:any[], baseOverride?:number, period?:any){
  adj = adj||[];
  ...
  // v155: PCB per LHDN MTD — annualise over the employee's ACTUAL service months in the tax year
  // (a mid-year joiner is taxed on the months they work, not a flat ×12), then LHDN rounding: truncate
  // to 2 dp → round UP to next 5 sen; a monthly MTD below RM10 is nil (not remitted).
  ...
  if(emp.resident===false){ pcb=myPcbRoundUp5(statWage*0.30); }
    ...
    const reliefs=rPers + (cat2?rSp:0) + kids*rCh + Math.min(epfEe*N, rEpf);
    const chargeable=Math.max(0, statWageNormal*N - reliefs);
    const tax=payProgTax(chargeable, bands);
    const rebate=chargeable<=35000 ? (400 + (cat2?400:0)) : 0;
    const monthlyBase=Math.max(0,(tax-rebate)/N);
    let addlTax=0;
    if(bonusStat>0){
      const chargeableB=Math.max(0, statWageNormal*N + bonusStat - reliefs);
  ...
  try {
    ...
    if (api === "hr_payroll_finalise") {
      ...
      for (const r of rows){
        ...
        const c = computePayrollMY(emp, cfg, adjE, baseOverride, { month: mo, year: yr });
        server.push({ eid, c });
