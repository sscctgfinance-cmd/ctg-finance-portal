...
export default function HrPayroll(p: HrPayrollProps) {
  ...
  return (
    <>
      ...
      {/* `return picker+hrRunsPanel()+employerPanel+ratesPanel+table+…` — hros.html:4204, in that
          order. The runs panel comes FIRST: it is the process list an operator picks a month from,
          so putting it under the two record editors would bury the thing the screen opens for. */}
      <RunsPanel p={p} />
      <EmployerPanel p={p} />
      <RatesPanel p={p} />
