...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (api === "hr_leave_admin") {
      ...
      if(ids.length){ const { data: st } = await sb.from("hr_leave_approval_steps").select("*").in("leave_request_id",ids).order("step_order"); await attachActorNames(st||[], "decided_by", "decided_by_name"); await attachAssignees(st||[], String(b.tenant||"")||null); (st||[]).forEach((s:any)=>{ (stepsByReq[s.leave_request_id]=stepsByReq[s.leave_request_id]||[]).push(s); }); }
