...
async function rcFallbackStep(tenant:string){
...
async function stepHasNoApprover(step:any, tenantId:any){
  if(!step) return true;
  if(step.approver_employee_id) return false;
  const role = String(step.approver_role||"").trim();
  if(!role) return true;                                  // plain unassigned
  ...
  const holders = (ras||[]).filter((r:any)=> !r.tenant_id || !tenantId || String(r.tenant_id)===String(tenantId));
  if(holders.length) return false;
  let q = sb.from("hr_employees").select("id",{count:"exact",head:true}).eq("claim_role",role);
  if(tenantId) q = q.eq("tenant_id",tenantId);
  const { count } = await q;                              // employees carrying the role directly
  return !(count||0);
}
async function canActOrGap(who:any, step:any, tenantId:any){
  if(rcCanActStep(who, step)) return true;
  if(!who.isAdmin) return false;
  return await stepHasNoApprover(step, tenantId);
}
...
async function rcApproverQueue(tenant:string, who:any){
  ...
  // v152: resolve once per distinct role whether ANYONE in this company holds it. A role with no holder
  // is an unfillable gap an admin may act on — otherwise these claims are invisible AND undecidable.
  const gapRole:any={};
  for(const r of Array.from(new Set((steps||[]).map((s:any)=>String(s.approver_role||"").trim()).filter(Boolean))))
    gapRole[r as string] = await stepHasNoApprover({ approver_role:r }, tenant);
  const canAct=(st:any)=> rcCanActStep(who, st) ||
    (who.isAdmin && (stepUnassigned(st) || (st.approver_role && gapRole[String(st.approver_role).trim()])));
  return (claims||[]).filter((c:any)=>{
    const st=byClaim[c.id]&&byClaim[c.id][c.current_step];
    if(!(st && st.status==="Pending" && canAct(st))) return false;
...
async function rcDecideOne(who:any, me:any, id:any, decision:string, comment:string, overrideAmount:any, overrideReason:string, pinTenants:any=null){
  ...
  if(!(await canActOrGap(who, step, claim.tenant_id)))
...
const MY_DEFAULT_TAX_BANDS: [number,number][] = [[5000,0],[20000,0.01],[35000,0.03],[50000,0.06],[70000,0.11],[100000,0.19],[400000,0.25],[600000,0.26],[2000000,0.28],[Infinity,0.30]];
function payRoundUp(n:number){ return Math.ceil(n-1e-9); }
...
function computePayrollMY(emp:any, cfg:any, adj:any[], baseOverride?:number){
  adj = adj||[];
  ...
  const sEe=scat===2?0:cfg.socso.eeRate, sEr=scat===2?(cfg.socso.erRate2!=null?cfg.socso.erRate2:0.0125):cfg.socso.erRate;
  const sp=socsoOn?payTableParts(statWage,cfg.socso.ceiling,sEe,sEr):{ee:0,er:0}; const socsoEe=sp.ee, socsoEr=sp.er;
  const eisOn=emp.eis_eligible!==false && !senior;
  const ip=eisOn?payTableParts(statWage,cfg.eis.ceiling,cfg.eis.eeRate,cfg.eis.erRate):{ee:0,er:0}; const eisEe=ip.ee, eisEr=ip.er;
  ...
  let pcb:number;
  if(emp.resident===false){ pcb=payRound2(statWage*0.30); }
  else {
    ...
    const reliefs=rPers + (cat2?rSp:0) + kids*rCh + Math.min(epfEe*12, rEpf);
    const chargeable=Math.max(0, statWageNormal*12 - reliefs);
    const tax=payProgTax(chargeable, bands);
    const rebate=chargeable<=35000 ? (400 + (cat2?400:0)) : 0;
    const monthlyBase=Math.max(0,(tax-rebate)/12);
    let addlTax=0;
    if(bonusStat>0){
      const chargeableB=Math.max(0, statWageNormal*12 + bonusStat - reliefs);
    ...
    pcb=payRound2(monthlyBase + addlTax);
  }
  ...
  try {
    ...
    if (api === "hr_leave_pending") {
      ...
      for(const r of pend){
        ...
        if(!(await canActOrGap(who, step, r.tenant_id))) continue;   // v152: unfillable role = a gap an admin may fill
    ...
    if (api === "hr_leave_decide") {
      ...
      if(!(await canActOrGap(who, step, req.tenant_id)))   // v152: unfillable role = a gap an admin may fill
    ...
    if (api === "hr_payroll_finalise") {
      ...
      for (const r of rows){
        ...
        const c = computePayrollMY(emp, cfg, adjE, baseOverride);
        server.push({ eid, c });
    ...
    if (api === "hr_rc_get") {
      ...
      const canAct = (await canActOrGap(who, curStep, cl&&cl.tenant_id)) && !sodOut && ["Submitted","Pending Manager Approval","Pending HR Approval","Pending Finance Approval","Pending Director Approval"].indexOf(cl&&cl.status)>=0;
